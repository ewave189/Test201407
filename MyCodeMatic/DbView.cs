...
namespace Codematic
{
    ...
    public partial class DbView : Form
    {
                ...
                case "sqlroot":
                    {
                        ...
                        ToolStripMenuItem 刪除所有dbItem = new ToolStripMenuItem();
                        刪除所有dbItem.Name = "刪除所有Item";
                        刪除所有dbItem.Text = "刪除所有";
                        刪除所有dbItem.Click += new EventHandler(刪除所有dbItem_Click);
                ...
                case "sql":
                    {
                        ...
                        ToolStripMenuItem 刪除Item = new ToolStripMenuItem();
                        刪除Item.Name = "刪除Item";
                        刪除Item.Text = "刪除sql";
                        刪除Item.Click += new EventHandler(刪除Item_Click);
        ...
        private void 新建查询Item_Click(object sender, EventArgs e)
        {
            ...
            //    TreeNode sqlNode;//  TreeClickNode.Nodes[2];
            //    if (TreeClickNode.Tag == "sqlroot")
            //    {
            //        dbname = TreeClickNode.Parent.Text;
            //        server = TreeClickNode.Parent.Parent.Text;
            //        sqlNode = TreeClickNode;
            //    }
            //    else
            //        sqlNode = TreeClickNode.Nodes[2];

            //    //MainForm mainfrm = (MainForm)Application.OpenForms["MainForm"];
            //    //AddTabPage(title, new DbQuery(mainfrm, ""), mainfrm);
            //    //mainfrm.toolComboBox_DB.Text = dbname;
            //    DbQuery2 qfrm = new DbQuery2();
            //    if (qfrm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            //    {
            //        //檢查 sql 合法和 名稱的唯一 
            //        LTP.CmConfig.DbSettings dbset = LTP.CmConfig.DbConfig.GetSetting(server);

            //        LTP.DbObjects.Oracle.DbObject oracleDB = new LTP.DbObjects.Oracle.DbObject(dbset.ConnectStr);
            //        bool bExistView = oracleDB.ExistView(qfrm.SqlName);
            //        try
            //        {
            //            if (oracleDB.ExistTab(qfrm.SqlName) && !bExistView)
            //                MessageBox.Show("名字已經存在!");
            //            else if (bExistView)
            //                LoadSqlView(qfrm.SqlName, oracleDB, sqlNode);
            //            else
            //            {
            //                oracleDB.CreateViewbySql(qfrm.Sql, qfrm.SqlName);
            //                if(qfrm .NameInfo !=null ||qfrm.ColumnInfos !=null )
            //                oracleDB.UpdateComments(qfrm.SqlName, qfrm.NameInfo, qfrm.ColumnInfos );
            //                LoadSqlView(qfrm.SqlName, oracleDB, sqlNode);
            //                // oracleDB.DropViewbySql(qfrm.SqlName);
            //            }
            //        }
            //        catch (Exception ex)
            //        {
        ...
        void 刪除Item_Click(object sender, EventArgs e)
        {
            //if (TreeClickNode == null)
            //    return;
            //string server = TreeClickNode.Parent.Parent.Parent.Text;
            //LTP.CmConfig.DbSettings dbset = LTP.CmConfig.DbConfig.GetSetting(server);
            //LTP.DbObjects.Oracle.DbObject oracleDB = new LTP.DbObjects.Oracle.DbObject(dbset.ConnectStr);
            //oracleDB.DropViewbySql(TreeClickNode.Text);
            //TreeClickNode.Remove();
        }
        void 刪除所有dbItem_Click(object sender, EventArgs e)
        {
            //if (TreeClickNode == null)
            //    return;
            //string server = TreeClickNode.Parent.Parent.Text;
            //LTP.CmConfig.DbSettings dbset = LTP.CmConfig.DbConfig.GetSetting(server);
            //LTP.DbObjects.Oracle.DbObject oracleDB = new LTP.DbObjects.Oracle.DbObject(dbset.ConnectStr);
            //foreach (TreeNode v in TreeClickNode.Nodes)
            //{
            //    oracleDB.DropViewbySql(v.Text);
            //}

            //TreeClickNode.Nodes.Clear();
        }
        //private void LoadSqlView(string name, LTP.DbObjects.Oracle.DbObject dbobj, TreeNode sqlNode)
        //{
        //    #region	sql视图
        ...
        //        string tabname = name;
        //        Dictionary<string, string> tabNamePair = dbobj.GetViewsEx(name );
        ...
        //        List<ColumnInfo> collist = dbobj.GetColumnList("", tabname);
        ...
        //    {
        //        LogInfo.WriteLog(ex);
        //        MessageBox.Show(this, "获取数据库" + "的视图信息失败：" + ex.Message, "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
        //    }
        //    #endregion
        //}
        ...
        private void 刷新dbItem_Click(object sender, EventArgs e)
        {
            ...
            //TreeNode sqlNode = new TreeNode("SQL");
            //sqlNode.ImageIndex = 3;
            //sqlNode.SelectedImageIndex = 4;
            //sqlNode.Tag = "sqlroot";
            //tn.Nodes.Add(sqlNode);
            ...
            //    Dictionary<string, string> tabNamePair = dbobj2.GetTablesEx(dbname);
            //    if (tabNames.Count > 0)
            //    {
        ...
        private void 生成ModelItem_Click(object sender, EventArgs e)
        {
            ...
            string dbname = TreeClickNode.Parent.Parent.Text;
            string tabname = TreeClickNode.Text;
            string tabcaption = TreeClickNode.ToolTipText;
            LTP.CodeBuild.CodeBuilders cb = ObjHelper.CreatCB(longservername);
            cb.DbName = dbname;
            cb.TableName = new LTP.IBuilder.SimplePair(tabname, tabcaption);
        ...
        private void CreatTree(string dbtype, string ServerIp, string constr, string Dbname, DoWorkEventArgs e)
        {
            ...
            ConnectServer(serverNode, dbtype, ServerIp, Dbname, true, e);
            #region Old
            ////0 serverlist
            ////1 server
            ////2 db
        ...
        private void ConnectServer(TreeNode serverNode, string dbtype, string ServerIp, string DbName, bool ConnectSimple, DoWorkEventArgs e)
        {
            ...
            foreach (TreeNode tn in serverNode.Nodes)
            {
                ...
                TreeNode sqlNode = new TreeNode("查詢");
                sqlNode.ImageIndex = 3;
                sqlNode.SelectedImageIndex = 4;
                sqlNode.Tag = "sqlroot";
                AddTreeNode(tn, sqlNode);
                ...
                try
                {
                    ...
                    Dictionary<string, string> tabNamePair = null;
                    try
                    {
                        tabNamePair = dbobj2.GetTablesEx(dbname);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message);
